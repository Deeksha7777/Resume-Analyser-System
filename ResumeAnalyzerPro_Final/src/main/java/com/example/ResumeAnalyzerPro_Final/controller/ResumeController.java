...
public class ResumeController {
    ...
    public String analyze(@RequestParam(value = "file", required = false) MultipartFile file,
        ...
        if (normalizedResumeContent.isBlank() || normalizedJobDesc.isBlank()) {
            populateDashboardModel(model, user, service.getLatestAnalysis(user));
            model.addAttribute("error", "Please upload a resume or paste resume text, and provide job description.");
            return "dashboard";
        }
        ...
        if (file != null && !file.isEmpty()) {
            ...
            fileName = "Pasted Resume";  // fallback
        }
        ...
        model.addAttribute("sourceFileName", resume != null ? resume.getFileName() : "Pasted Resume Content");
        return "dashboard";
    }
    ...
    @ResponseBody
    public ResponseEntity<HistoryChunkResponse> loadMoreHistory(@RequestParam(value = "offset", defaultValue = "10") int offset,
                                                                HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
    ...
    private byte[] buildAnalysisPdf(Analysis analysis, String userName) {
        ...
        document.add(new Paragraph("Generated: " + analysis.getCreatedAt().format(DateTimeFormatter.ofPattern("dd MMM yyyy, hh:mm a"))));
